...
export class MultimodalLiveClient extends EventEmitter<MultimodalLiveClientEventTypes> {
      ...
      if (evt.data instanceof Blob) {
        ...
        try {
          ...
          if (jsonData.status) {
            ...
            this.emit('status', jsonData.status);
          }
          if (jsonData.groundingResponse || (jsonData.modelTurn && jsonData.modelTurn.parts)) {
            this.emit('content', jsonData);
          }
          if (jsonData.serverContent && jsonData.serverContent.turnComplete) {
            this.emit('turncomplete');
          }
          if (jsonData.serverContent && jsonData.serverContent.interrupted) {
    ...
    if (isServerContenteMessage(response)) {
      ...
      if (isModelTurn(serverContent)) {
        ...
        const audioParts = parts.filter(
          p => (p as any).inlineData && (p as any).inlineData.mimeType.startsWith('audio/pcm')
        );
  ...
  sendRealtimeInput(chunks: { data: string, mimeType: string }[]) {
