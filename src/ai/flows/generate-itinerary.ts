...
- **Rejection**: Discard any places located in other cities or regions mentioned in the video (e.g., if the destination is "Athens Georgia", discard any locations listed in Atlanta or Helen Georgia).
...
For each location, provide only its name and a short description. Do NOT include an address or image URL; that will be found later. Ensure each location name refers to a single, distinct place. Do not combine multiple separate places into a single entry (e.g., do not use 'New College and Moore College' if they are distinct buildings).
      ...
      itineraryOutput.itinerary.map(async (day) => {
        const locationsWithDetails = await Promise.all(
          day.locations.map(async (location) => {
            try {
              const query = (destinationLat && destinationLng) ? location.name : `${location.name}, ${targetDestination}`;

              const place = await findPlaceTool({ query: query, lat: destinationLat, lng: destinationLng });
              return {
                ...location,
